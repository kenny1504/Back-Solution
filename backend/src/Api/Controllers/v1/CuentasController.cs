...
public sealed class CuentasController : ControllerBase
{
  ...
  private readonly IValidator<CreateCuentaDto> _createCuentaValidator;
  private readonly IValidator<UpdateCuentaDto> _updateCuentaValidator;
    ...
    IValidator<UpdateCuentaDto> updateV)
  {
    ...
    _createCuentaValidator = createV;
    _updateCuentaValidator = updateV;
  }
  ...
  {
    var validationResult = await _createCuentaValidator.ValidateAsync(dto, ct);
  ...
  {
    var validationResult = await _updateCuentaValidator.ValidateAsync(dto, ct);
    if (!validationResult.IsValid)
    {
      var validationErrors = validationResult.Errors
        .GroupBy(error => error.PropertyName)
